...
class PathScannerTest(unittest.TestCase):
    ...
    def test_file_with_all_words_file(self):
        """Tests file_with_all_words(path, word_list). """
        # Edge case for the file with all words function
        # Instead of getting a directory we got a file
        # So we just need to check if the given file has all of the words


        result = os.path.join("test", "a", "b", "c", "c_file")
        root = os.path.join("test", "a")
        self.assertEqual(result, file_with_all_words(os.path.join(root,"b","c","c_file"), self._word_list))
